...
def about(topic):
    ...
    "*** YOUR CODE HERE ***"
    def ret_fc(paras):
        #把string转换为一个个单词，用topic每一个词去匹配
        def str_to_word(string):
            ls = []
            left = 0
            right = 0
            while right <= len(string):
                if right == len(string) or string[right] == ' ':#以空格为分割
                    tmp = ''
                    for i in range(left, right):
                        if isalpha(string[i]):#去掉不是英文字母的符号
                            tmp += string[i]
                    if tmp:
                        ls += [tmp.lower()]
                    left = right + 1
...
def accuracy(typed, reference):
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    if not len(typed_words) and not len(reference_words):
        return 100.0
    if not len(typed_words) or not len(reference_words):
        return 0.0
    cnt = 0
    i = 0
    j = 0
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for wd in word_list:
        if typed_word == wd:
            return wd
        diff = diff_function(typed_word, wd, limit)
        if diff <= limit and diff < min:
            min = diff
            ret_wd = wd
    if not ret_wd:
        return typed_word
...
def feline_fixes(typed, reference, limit):
    ...
    # BEGIN PROBLEM 6
    '''尚未解决在超过limit时立刻停止递归'''
    #print("call_count =", plusone.call_count)
    ...
    if plusone.call_count > limit:
        return limit + 1
    return plusone() + feline_fixes(typed[1:], reference[1:], limit)
...
def report_progress(typed, prompt, user_id, upload):
    ...
    while i < len(typed):
        ...
        i += 1
    prg = cnt / len(prompt)
    upload({'id': user_id, 'progress':prg})
    return prg
    # END PROBLEM 8
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    wordlist = [[]] * len(match["times"])
    for w_idx in word_indices:
        mintime = 114514
        for player in player_indices:
            t = time(match, player, w_idx)
            if t < mintime:
                mintime = t
                fast = player
        wordlist[fast] = wordlist[fast] + [get_word(match, w_idx)]
        #wordlist[fast] += [get_word(match, w_idx)]
        #wordlist[fast] += [get_word(match, w_idx)]无法通过，why?????
        '''
        原因是对于list, lits += blabla 和list = list + blabla 有点不同.后者会在内存中新建一个list,
        也即list在内存中地址变了,而前者是在原来的地址上的list做变化
        '''
    return wordlist
    # END PROBLEM 10
