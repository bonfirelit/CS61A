...
'''除了LaserAnt之外的全部问题都已通过,2022/8/8'''
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not tmp_place.is_hive:
            if self.lower_bound <= distance <= self.upper_bound and tmp_place.bees != []:
                return random_bee(tmp_place.bees)
            distance += 1
            tmp_place = tmp_place.entrance
        return None
        # END Problem 3 and 4
        '''关于问题4,最开始准备在每个子类里单独写一个nearest_bee函数,但题目要求不能另写。
        所以在父类中改写,但却没有在父类中定义upper_bound和lower_bound,就只能在函数中写if语句,写得很长且没过测试
        ,最后终于想到在父类中也定义upper和lower,只需设定不同值就可达到设置不同范围的目的'''
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        '''用for循环遍历list且中途调用remove删除了某个值,被删除的值的后一个元素不会被访问到,如何解决??
        答案:for bee in self.place.bees[:],百度解决的
        '''
        for bee in self.place.bees[:]: #wrong: for bee in self.place.bees:
            if amount >= self.health:   #fireant die
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.turns_to_chew == 0: #ant are not chewing bees
            bee = random_bee(self.place.bees)
            if bee: #there is a bee
    ...
    def action(self, gamestate):
        ...
        while tmp_place != None:
            if tmp_place.ant is not None:
                if tmp_place.ant.is_container:
                    tmp_place.ant.double()
                    tmp_place.ant.doubled = True
                    if tmp_place.ant.ant_contained is not None:
                        tmp_place.ant.ant_contained.double()
                        tmp_place.ant.ant_contained.doubled = True
                else:
                    tmp_place.ant.double()
                    tmp_place.ant.doubled = True
