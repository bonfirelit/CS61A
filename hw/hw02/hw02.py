...
def accumulate(merger, start, n, term):
    ...
    def func(num):
        ...
        if num == 1:
            ...
            return merger(term(num), func(num - 1))
    return merger(start, func(n))
...
def filtered_accumulate(merger, start, cond, n, term):
    ...
    def merge_if(x, y):
        "*** YOUR CODE HERE ***"
        if cond(x) == True:
            #print('x is', x, 'y is', y, 'will merger')
            return merger(x, y)
        elif cond(x) == False and cond(y) == True:
            #print('x is',x,'y is',y,'and x false, y true, will return', y)
            return y
        elif x is start:
            #print('x is start y is',y, 'return start')
            return start
        else:
            #print('x is',x,'y is',y,'and they both false, so return y')，因为y可能是merger后的结果
            return y
...
def funception(func_a, start):
    ...
    def func_b(stop):
        if start < 0:
            ...
            production = 1
            i = start #不能在循环中对start+=1,需要一个变量存储start
            while i < stop:
                production *= func_a(i)
                i += 1
            return production
