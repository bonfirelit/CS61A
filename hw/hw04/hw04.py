...
def remove_odd_indices(lst, odd):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(lst, odd, idx):
        if idx == len(lst):
            return []
        if odd and idx % 2 == 1:
            return helper(lst, odd, idx + 1)
        if odd and idx % 2 == 0:
            return [lst[idx]] + helper(lst, odd, idx + 1)
        if not odd and idx % 2 == 0:
            return helper(lst, odd, idx + 1)
        if not odd and idx % 2 == 1:
            return [lst[idx]] + helper(lst, odd, idx + 1)
    return helper(lst, odd, 0)
